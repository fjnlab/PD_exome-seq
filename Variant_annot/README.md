# Variant Annotation
Single nucleotide variants and small insertions and deletions were annotated using the Ensembl Variant Effect Predictor (VEP) software. Variant annotation was based on canonical transcripts as defined in Ensembl VEP. Population-level frequencies from Genome Aggregation Database (gnomAD) were added using ANNOVAR. Pathogenicity predictions for frameshifts and splice variants were generated by LOFTEE as part of the VEP pipeline. Pathogenicity predictions for missense variants annotated with PolyPhen-2 webtool. 
...
## Usage
### 1. Generate unique identifier for VEP submission from `variants.filtered_DPGQ.vcf.gz`. (Already done in steps prior to Variant filtering, see step #2)
```bash
./run_uniqueID_vcf.sh
```
